    ...
    public class FTPAction
    {
        ...
        public static bool getFile(string Ihost, string Iuser, string Ipassword, string localPath, string localFileName, string pathfile, string SafeFileName)
        {
            ...
            try
            {
                ...
                LogFile.writeLog(LogFile.DIR, "FTPRecived_" + LogFile.getTimeStringNow() + ".txt", LogFile.Filemode.GHIDE, "FILE :" + localPath + "/" + localFileName + "(response.StatusDescription)");
                CheckExistDirandCreate(pathfile);
